...
void Bureaucrat::execute_form(const AForm &f) const{
	try{
		f.execute(*this);
		std::cout << this->_name << " executed " << f.get_name() << std::endl;
	}
	catch(std::exception &e){
		std::cout << this->_name << " couldn't execute " << f.get_name() << " because " << e.what() << std::endl;
	}
	return;
}
